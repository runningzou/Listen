package com.runningzou.listen.base.rxjava;
...
public class RxCommand<T,V> {

    public static <T,V> RxCommand<T,V> create(Function<V, Observable<T>> function) {
        return new RxCommand<>(function);
    }

    public static <T,V> RxCommand<T,V> create(Observable<Boolean> enabled, Function<V, Observable<T>> function) {
        return new RxCommand<>(enabled, function);
    }

    private final Function<V, Observable<T>> mFunc;
     ...
     */
    public RxCommand(@Nullable Observable<Boolean> enabledObservable, @NonNull Function<V, Observable<T>> func) {
     ...
     * @param func
     */
    public RxCommand(Function<V, Observable<T>> func) {
        this(null, func);
    }
     ...
     */
    @MainThread
    public final Observable<T> execute(@Nullable V input) {
